...
我们计划弃用 `handleHotUpdate` 插件钩子，转而使用 [`hotUpdate` 钩子](/guide/api-environment-plugins#the-hotupdate-hook)，使其能够感知 [Environment API](/guide/api-environment.md)，并通过 `create` 和 `delete` 处理其他监视事件。
