...
本页面面向运行时提供方，即将 JavaScript 运行时与 Vite 集成的作者。这里的运行时是指转换后代码执行的 JavaScript 引擎，例如 Node.js、浏览器、Cloudflare 的 workerd，或 Worker 线程。运行时提供方会为这些运行时之一打包集成，因此框架作者和最终用户（构建应用的开发者）无需自行设置。

## 环境工厂

环境工厂应由运行时提供方实现，而不是最终用户。环境工厂会返回一个 `EnvironmentOptions`，适用于在开发和构建环境中都使用目标运行时的最常见场景。默认环境选项也可以预先设置，这样用户就不需要自己配置。
...
digraph module_runner {
  ...
  subgraph cluster_server {
    label="Vite 开发服务器（Node.js）" labeljust=l fontname="Arial" fontsize=12
    style="rounded,filled" fillcolor="${#f6f6f7|#1a1a1f}" color="${#c2c2c4|#3c3f44}"
    fontcolor="${#3c3c43|#ffffff}"

    subgraph cluster_env {
      label="开发环境" labeljust=l fontname="Arial" fontsize=11
      style="rounded,filled" fillcolor="${#f2ecfc|#2c273e}" color="${#c2c2c4|#3c3f44}"
      fontcolor="${#3c3c43|#ffffff}"

      plugins [label="插件\n管线" fillcolor="${#e9eaff|#222541}"]
      mg [label="模块\n图" fillcolor="${#e9eaff|#222541}"]
      hot [label="热通道" fillcolor="${#fcf4dc|#38301a}"]
  ...
  subgraph cluster_runtime {
    label="目标运行时" labeljust=l fontname="Arial" fontsize=12
    style="rounded,filled" fillcolor="${#f0fdf4|#131b15}" color="${#c2c2c4|#3c3f44}"
    fontcolor="${#3c3c43|#ffffff}"

    subgraph cluster_runner {
      label="模块运行器" labeljust=l fontname="Arial" fontsize=11
      style="rounded,filled" fillcolor="${#def5ed|#15312d}" color="${#c2c2c4|#3c3f44}"
      fontcolor="${#3c3c43|#ffffff}"

      evaluator [label="模块\n求值器" fillcolor="${#def5ed|#15312d}"]
      transport [label="传输" fillcolor="${#fcf4dc|#38301a}"]
    }
  }

  hot -> transport [label="HMR / 模块\n获取与调用" dir=both style=bold color="${#6f42c1|#c8abfa}"]
}
```
...
对于在与 Vite 服务器相同的 Node.js 进程中运行的 SSR 环境，Vite 提供了现成可用的 `createServerHotChannel` 作为 `HotChannel`：
