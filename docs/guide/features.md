...
此选项仅部分支持。完整支持需要 TypeScript 编译器进行类型推断，而这并不受支持。详情请参见 [Oxc Transformer 的文档](https://oxc.rs/docs/guide/usage/transformer/typescript.html#decorators)。
...
默认情况下，glob 模式匹配区分大小写。你可以使用 `caseSensitive` 选项来更改此行为：
...
设置 `caseSensitive: false` 后，glob 会匹配任意大小写的文件（例如，`module*.js` 会匹配 `Module.js`、`module.js`、`MODULE.js`）。
...
Vite 支持通过两种方式导入预编译的 `.wasm` 文件：当你只需要模块导出时，直接作为 [ES 模块](#esm-integration) 导入；或者在你需要对实例化进行显式控制时，使用 [`?init`](#manual-initialization)。

### ESM 集成

`.wasm` 文件可以直接导入。Vite 会从二进制中读取模块的导入和导出，实例化它，并将其导出重新作为命名 ES 模块导出暴露：
...
如果 WebAssembly 模块自身声明了导入项，Vite 会从 JavaScript 模块中解析它们。每个导入项的模块名都被视为一个导入说明符（相对于 `.wasm` 文件解析），所请求的成员会自动接入实例。
